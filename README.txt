...
- When an enemy reaches the edge of the window, it should bounce off in
  the oposite direction at the same speed.
...
Lines will be given in thet order, with the following syntax:
...
- This line declares that the SFML Window must be constructed with width W
  ...
  display the application in full-screen mode (1) of not (0).
...
I recommend approacing this assignment in the following order:

0. Save the configuration file reading until later, after Entities implemented  X
1. Implement the Vec2 class, which you will use for all Components              X
2. Implement the basic functionality in the EntityManager class                 X
   Implement the EntityManager's addEntity() and update() functions so          X
   you can start testing the Game class quickly. Don't worry about the update() X
   function which deletes dead entities until later when you get more game      X
   mechanics working.                                                           X
3. Implement basics of the Game class:                                          X
   a. Construct a player Entity using the spawnPlayer() function                X
   b. Implement basic drawing of entities using the Game::sRender() function    X
   c. Construct some enemies using the spawnEnemy() function                    X
   d. Construct a bullet using the spawnBullet() function                       X
4. Implement Player Movement in Game::sUserInput and Game::sMovement            X
5. Implement the EntityManager::update() function so it deletes dead entities   X
6. Implement the EntityManager::getEntities(tag) functionality                  X
7. Implement collisions in sCollision and entity.destroy() if it's dead         X
